...
class DemoKeyboardActionHandler: StandardKeyboardActionHandler {
  ...
  var prevKeyboardType: KeyboardType = KeyboardType.alphabetic(uppercased: false)
    ...
    override func tapAction(for action: KeyboardAction, view: UIView) -> GestureAction? {
        switch action {
          ...
          case .switchToKeyboard(let type): return { [weak self] in
            self?.prevKeyboardType =
              self?.demoViewController?.keyboardType ?? KeyboardType.alphabetic(uppercased: false)
            self?.demoViewController?.keyboardType = type
          }
          case .toolbar: return { [weak self] in
            self?.prevKeyboardType =
              self?.demoViewController?.keyboardType ?? KeyboardType.alphabetic(uppercased: false)
            self?.demoViewController?.keyboardType = KeyboardType.custom("ResultKeyboard")
          }
          case .back: return { [weak self] in
            self?.demoViewController?.keyboardType = (self?.prevKeyboardType)!
          }
