...
extension KeyboardViewController {
  ...
  func buttonRow(
    ...
    let result = KeyboardButtonRow(actions: actions, distribution: distribution) {
  ...
  func toolbarButton(action: KeyboardAction, distribution: UIStackView.Distribution = .equalSpacing) -> UIView {
    let view = ToolbarButton.fromNib(owner: self)
    view.setup(with: action, in: self, distribution: distribution)
    return view
  }
